...
const hardwaresItemsList1 = ref('')
const hardwaresItemsList2 = ref('')
const appsItemsList1 = ref('')
const appsItemsList2 = ref('')
const teachingResourcesItemsList = ref('')
const vrVideosItemsList = ref('')

//heading text variables
const rr_text = ref('Required resources')
const or_text = ref('Other resources to try (optional)')
...
const handleFileUpload = async (event) => {
    ...
    try {
        ...
        hardwaresItemsList1.value = extractItemsByKeyword(htmlContent.value, 'Hardware', '', rr_text.value);
        hardwaresItemsList2.value = extractItemsByKeyword(htmlContent.value, 'Hardware', '', or_text.value);
        appsItemsList1.value = extractItemsByKeyword(htmlContent.value, 'Apps', 'App:', rr_text.value);
        appsItemsList2.value = extractItemsByKeyword(htmlContent.value, 'Apps', 'App:', or_text.value);
        console.log(hardwaresItemsList1.value)
        console.log(hardwaresItemsList2.value)
        console.log(appsItemsList2.value.ItemsForRR_2.text)
...
const downloadFormattedJson = () => {
    jsonContent.value = JSON.stringify({
        ...
        "Required Resources": {
            "HardwareS1": hardwaresItemsList1.value.ItemsForRR_1,
            "HardwareS2": hardwaresItemsList1.value.ItemsForRR_2,
            "AppsS1": appsItemsList1.value.ItemsForRR_1,
            "AppsS2": appsItemsList2.value.ItemsForRR_2,
            "Links": (appsItemsList1.value.LinksForAll + appsItemsList2.value.LinksForAll),
        ...
        "Other resources to try (optional)": {
            "HardwareS1": hardwaresItemsList2.value.ItemsForOR_1,
            "HardwareS2": hardwaresItemsList2.value.ItemsForOR_2,
            // You can add other required resources here if needed
...
const extractItemsByKeyword = (html, keyword1, keyword2, headingKey) => {
    const doc = new DOMParser().parseFromString(html, 'text/html');
    ...
    for (let i = 0; i < trTags.length; i++) {
        ...
        if (h1Tags.length > 0 && strongTags.length > 0) {
            ...
            if (sessionText.includes(headingKey)) {
                    ...
                    if (strongTag.textContent.trim() === keyword1 + ':' || strongTag.textContent.trim() === keyword1 || strongTag.textContent.trim() === keyword2) {
                        ...
                        if (ulTag && ulTag.tagName.toLowerCase() === 'ul') {
                            ...
                            liTags.forEach((liTag, index) => {
                                ...
                                const isRR = headingKey === rr_text.value;
                                const isOR = headingKey === or_text.value;
                                ...
                                if (aTag) {
                                    ...
                                    const list_text = liTag.textContent.trim();
                                    if (inSession1 && isRR) {
                                        ItemsForRR_1.push({ text, href, list_text });
                                    } else if (inSession2 && isRR) {
                                        ItemsForRR_2.push({ text, href, list_text });
                                    } else if (inSession1 && isOR) {
                                        ItemsForOR_1.push({ text, href, list_text });
                                    } else if (inSession2 && isOR) {
                                        ItemsForOR_2.push({ text, href, list_text });
                                    }
                                } else {
                                    const content = liTag.textContent.trim();
                                    if (inSession1 && isRR) {
                                        ItemsForRR_1.push(content);
                                    } else if (inSession2 && isRR) {
                                        ItemsForRR_2.push(content);
                                    } else if (inSession1 && isOR) {
                                        ItemsForOR_1.push(content);
                                    } else if (inSession2 && isOR) {
                                        ItemsForOR_2.push(content);
                                    }
                                }
