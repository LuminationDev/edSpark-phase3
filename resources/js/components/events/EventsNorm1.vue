...
const currentUserEMSlink = ref(false)
...
onMounted(() => {
  ...
  //     currentUserEMSlink.value = res.data['rsvped'] === 'true'
...
const handleSubmitRsvp = () => {
  ...
  if (!!v$.value.$errors) {
    ...
    return axios.post(API_ENDPOINTS.EVENT.ADD_RSVP_TO_EVENT, rsvpData).then(res => {
      ...
      currentUserEMSlink.value = false
...
<template>
  <div class="EventRsvpFormContainer bg-blue-900 mt-5 px-4 py-4 text-white">
    ...
    <!--        <div-->
    <!--             v-if="(currentUserIsOwner && currentUserEMSlink && eventStatus !== 'ENDED') || editingEMSlink"-->
    ...
    >
      <div class="-mt-1 Selector dropdown flex flex-col school">
        <label class="-mb-2 ml-2"> You have provided the Link<br/><br/><i>https://www.edspark.com.au/....</i></label>
    ...
    <!--    Form no 2 - conditional, user=owner && EMS=no-->
    <form
        v-if="(currentUserIsOwner && !currentUserEMSlink && eventStatus !== 'ENDED') || editingEMSlink"
        class="border-b-2 border-white flex flex-col gap-2 py-8 rsvpFormInputs"
        autocomplete="off"
    ...
    <!--        Form no 4 - conditional, user=Not-owner && EMS=No-->
    <form
        v-if="(!currentUserIsOwner && !currentUserEMSlink && eventStatus !== 'ENDED') || editingEMSlink"
        class="border-b-2 border-white flex flex-col gap-2 py-8 rsvpFormInputs"
        autocomplete="off"
    ...
    <!--        <form-->
    <!--            v-else-if="(!currentUserIsOwner && !currentUserRsvped && eventStatus !== 'ENDED') || editingEMSlink"-->
    <!--            class="border-b-2 border-white flex flex-col gap-2 py-8 rsvpFormInputs"-->
    <!--            autocomplete="off"-->
    <!--            @submit.prevent=""-->
    <!--        >-->
