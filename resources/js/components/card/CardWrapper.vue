...
<template>
    <div>
        ...
        <div v-if="!loading && loadingState === 'SUCCESS' || !loading && loadingState === 'VALIDATING' || (typeof loadingState === 'boolean' && !loadingState)"
            class="card__wrapper" :class="additionalClasses + hasInfoSection ? '' : ''">
            <div class="flex flex-row flex-1 flex-wrap w-full justify-between px-[29px]">
                <div v-for="(col, index) in (props.rowCount * props.colCount)" class="!max-w-[400px] w-[400px] mb-[42px]"
                    :class="additionalClasses">
                    <GenericCard v-if="computedCardData" :key="computedCardData[index].id" :item="computedCardData[index]"
                        :title="computedCardData[index].title" :display-content="computedCardData[index].excerpt"
                        :display-author="computedCardData[index].author" :display-date="computedCardData[index].created_at"
                        :cover-image="computedCardData[index].cover_image" :number-per-row="2"
                        ...
                        :section-type="sectionType" @emit-card-click="handleClickCard">
                        <template v-if="sectionType === 'advice'" #typeTag>
                            <div v-if="computedCardData[index].type === 'D.A.G advice'"
                            ...
                            <div v-else-if="computedCardData[index].type === 'Your Classroom'"
                                class="TypeTag absolute gap-4 -right-6 top-4 p-1 px-6 h-[39px] place-items-center bg-secondary-green text-white flex rounded"
                                :class="typeTagColor">
                                {{ computedCardData[index].type }}
                            </div>

                            <div v-else-if="computedCardData[index].type === 'Your Work'"
                                class="TypeTag absolute gap-4 -right-6 top-4 p-1 px-6 h-[39px] place-items-center bg-secondary-green text-white flex rounded"
                                :class="typeTagColor">
                                {{ computedCardData[index].type }}
                            </div>

                            <div v-else-if="computedCardData[index].type === 'Your Learning'"
                                class="TypeTag absolute gap-4 -right-6 top-4 p-1 px-6 h-[39px] place-items-center bg-secondary-green text-white flex rounded"
                                :class="typeTagColor">
                                {{ computedCardData[index].type }}
                            </div>

                            <div v-if="computedCardData[index].type === 'Partner'"
                                class="TypeTag absolute gap-4 -right-6 top-4 p-1 px-6 h-[39px] place-items-center bg-secondary-yellow text-white flex rounded"
                                :class="typeTagColor">
                                {{ computedCardData[index].type }}
                            </div>
                        ...
                        <template #icon>
                            <SoftwareCardIcon class="icon absolute -top-6 -right-6 "
        ...
        <div v-else>
            <CardLoading :number-of-rows="rowCount" :number-per-row="colCount" />
        </div>
    </div>
</template>
